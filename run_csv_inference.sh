export PATH=/gxr/jixian/anaconda3/envs/relax_new/bin:$PATH
# python='python'
...
# header='1qg8_shrink'
fileName='test_af2.csv'
raw_dataFile='data/'${fileName}
header='test_af2_relax_opt'

python='/gxr/jixian/anaconda3/envs/diffdynamic/bin/python'
relax_python='/gxr/jixian/anaconda3/envs/relax_new/bin/python'
seed=8

dataFile='data/tmp/'${fileName}
CUDA_VISIBLE_DEVICES=$1 $relax_python -m clean_pdb --protein_ligand_csv $raw_dataFile $dataFile
...
CUDA_VISIBLE_DEVICES=$1 $python -m inference --ckpt ema_inference_epoch314_model.pt --protein_dynamic --save_visualisation --model_dir workdir/big_score_model_sanyueqi_with_time --seed $seed  --protein_ligand_csv $dataFile --esm_embeddings_path data/esm2_output --out_dir results/$header\_seed$seed --inference_steps 20 --samples_per_complex 40 --savings_per_complex 40 --batch_size 5 --actual_steps 20 --no_final_step_noise

CUDA_VISIBLE_DEVICES=$1 $relax_python -m relax_final --results_path results/$header\_seed$seed --samples_per_complex 40
