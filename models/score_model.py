...
class TensorProductScoreModel(torch.nn.Module):
    def __init__(self, t_to_sigma, device, timestep_emb_func, in_lig_edge_features=4, sigma_embed_dim=32, sh_lmax=2,
                 ns=16, nv=4, num_conv_layers=2, lig_max_radius=5, rec_max_radius=15, c_alpha_max_neighbors=24, cross_max_distance=250,
        ...
        if self.confidence_mode:
            ...
            self.final_conv = TensorProductConvLayer(
                ...
                sh_irreps=self.sh_irreps,
                out_irreps=f'{2*ns}x0e + 2x1o + 2x1e',
            ...
            self.lddt_final_layer = nn.Sequential(nn.Linear(ns, ns),nn.Dropout(dropout), nn.ReLU(), nn.Linear(ns, 1), nn.Sigmoid())
            self.affinity_final_layer = nn.Sequential(nn.Linear(ns, ns),nn.Dropout(dropout), nn.ReLU(), nn.Linear(ns, 1), nn.ReLU())
            self.tr_final_layer = nn.Sequential(nn.Linear(1 + self.sigma_embed_dim, ns),nn.Dropout(dropout), nn.ReLU(), nn.Linear(ns, 1))
            self.rot_final_layer = nn.Sequential(nn.Linear(1 + self.sigma_embed_dim, ns),nn.Dropout(dropout), nn.ReLU(), nn.Linear(ns, 1))

            # self.res_final_conv = TensorProductConvLayer(
            #     in_irreps=self.rec_conv_layers[-1].out_irreps,
            #     sh_irreps=self.sh_irreps,
    ...
    def forward(self, data, eps=1e-12):
        if not self.confidence_mode:
            tr_sigma, rot_sigma, tor_sigma, res_tr_sigma, res_rot_sigma, res_chi_sigma = self.t_to_sigma(*[data.complex_t[noise_type] for noise_type in ['tr', 'rot', 'tor', 'res_tr', 'res_rot', 'res_chi']])
        else:
            tr_sigma, rot_sigma, tor_sigma, res_tr_sigma, res_rot_sigma, res_chi_sigma = [data.complex_t[noise_type] for noise_type in ['tr', 'rot', 'tor', 'res_tr', 'res_rot', 'res_chi']]
        # build ligand graph
        ...
        if self.dynamic_max_cross:
            # cross_cutoff = (tr_sigma * 3 + 12).unsqueeze(1)
            cross_cutoff = torch.maximum(torch.tensor(20).float().to(tr_sigma.device),(tr_sigma * 3 + 12).unsqueeze(1))
            # print(cross_cutoff, "out")
        else:
        ...
        lddt_pred = self.lddt_final_layer(global_pred[:,:self.ns])
        affinity_pred = torch.clamp(self.affinity_final_layer(global_pred[:,self.ns:2*self.ns]) / (lddt_pred+eps),min=0.0,max=15.0)
        if self.finetune:
            return affinity_pred

        # tr_pred = global_pred[:, 2:5] + global_pred[:, 8:11]
        # rot_pred = global_pred[:, 5:8] + global_pred[:, 11:]
        tr_pred = (global_pred[:, 2*self.ns:2*self.ns+3] + global_pred[:, 2*self.ns+3:2*self.ns+6])
        rot_pred = global_pred[:, 2*self.ns+6:2*self.ns+9] + global_pred[:, 2*self.ns+9:]
        # print(tr_pred)
        # print(rot_pred)
        ...
        tr_pred = tr_pred / (tr_norm+eps) * self.tr_final_layer(torch.cat([tr_norm, data.graph_sigma_emb], dim=1))
        rot_norm = torch.linalg.vector_norm(rot_pred, dim=1).unsqueeze(1)
        rot_pred = rot_pred / (rot_norm+eps) * self.rot_final_layer(torch.cat([rot_norm, data.graph_sigma_emb], dim=1))

        # compute translational and rotational score vectors
        # res_global_pred = self.res_final_conv(rec_node_attr, rec_edge_index, rec_edge_attr, rec_edge_sh, out_nodes=rec_node_attr.shape[0])

        # res_tr_pred = (rec_node_attr[:, self.ns:self.ns+self.nv//2*3] + rec_node_attr[:, self.ns+self.nv*3:self.ns+(self.nv+self.nv//2)*3]).view(rec_node_attr.shape[0],-1,3).mean(1)
        # res_rot_pred = (rec_node_attr[:, self.ns+self.nv//2*3:self.ns+self.nv*3] + rec_node_attr[:, self.ns+(self.nv+self.nv//2)*3:self.ns+(self.nv+self.nv)*3]).view(rec_node_attr.shape[0],-1,3).mean(1)
        ...
        if self.scale_by_sigma:
            ...
            # rot_pred = rot_pred * rot_sigma.unsqueeze(1)#so3.score_norm(rot_sigma.cpu()).unsqueeze(1).to(data['ligand'].x.device)


        if self.no_torsion or data['ligand'].edge_mask.sum() == 0: return lddt_pred, affinity_pred, tr_pred, rot_pred, torch.empty(0, device=self.device), res_tr_pred, res_rot_pred, res_chi_pred
        ...
        if data['ligand'].edge_mask.sum() > 0:
            ...
            tor_t = data.complex_t['tor'][data['ligand'].batch][data['ligand', 'ligand'].edge_index[0]][data['ligand'].edge_mask]
            tor_sigma_emb = self.timestep_emb_func(tor_t)
            ...
            tor_pred = []
        return lddt_pred, affinity_pred, tr_pred, rot_pred, tor_pred, res_tr_pred, res_rot_pred, res_chi_pred
    ...
    def build_rec_conv_graph(self, data):
        ...
        node_attr = torch.cat([data['receptor'].x, data['receptor'].chis.sin()*data['receptor'].chi_masks, data['receptor'].chis.cos()*data['receptor'].chi_masks, data['receptor'].node_sigma_emb], 1)

        # this assumes the edges were already created in preprocessing since protein's structure is fixed
        edge_index = radius_graph(data['receptor'].pos, self.rec_max_radius, data['receptor'].batch, max_num_neighbors=self.c_alpha_max_neighbors)
        edge_index = edge_index[[1,0]]
        # print(edge_index)
        # print(len(torch.unique(edge_index[0])),len(torch.unique(edge_index[1])),data['receptor'].pos.shape)
        # print(set(torch.unique(edge_index[0]).detach().cpu().numpy())^set(torch.unique(edge_index[1]).detach().cpu().numpy()))
        # print(len(torch.unique(data['receptor', 'receptor'].edge_index[0])),len(torch.unique(data['receptor', 'receptor'].edge_index[1])))
    ...
    def build_cross_conv_graph(self, data, cross_distance_cutoff):
        ...
        if len(torch.unique(data['ligand'].batch[edge_index[0]])) < data.num_graphs:
