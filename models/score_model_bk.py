...
class TensorProductScoreModel(torch.nn.Module):
    ...
    def forward(self, data):
        if not self.confidence_mode:
            tr_sigma, rot_sigma, tor_sigma, res_tr_sigma, res_rot_sigma = self.t_to_sigma(*[data.complex_t[noise_type] for noise_type in ['tr', 'rot', 'tor', 'res_tr', 'res_rot']])
        else:
            tr_sigma, rot_sigma, tor_sigma, res_tr_sigma, res_rot_sigma = [data.complex_t[noise_type] for noise_type in ['tr', 'rot', 'tor', 'res_tr', 'res_rot']]
        if self.scale_by_sigma:
            res_tr_sigma = res_tr_sigma * data['receptor'].af2_trans_sigma
            res_rot_sigma = res_rot_sigma * data['receptor'].af2_rotvecs_sigma
        ...
        for l in range(len(self.lig_conv_layers)):
            ...
            if l == 0:
                ...
                n_norm_vec = n_vec / n_vec.norm(dim=-1,keepdim=True)
                c_vec = data['receptor'].lf_3pts[:,2] - data['receptor'].lf_3pts[:,1]
                c_norm_vec = c_vec / c_vec.norm(dim=-1,keepdim=True)
        ...
        res_tr_pred = res_tr_pred / res_tr_norm * self.res_tr_final_layer(torch.cat([res_tr_norm, data.graph_sigma_emb], dim=1))
        res_rot_norm = torch.linalg.vector_norm(res_rot_pred, dim=1).unsqueeze(1)
        res_rot_pred = res_rot_pred / res_rot_norm * self.res_rot_final_layer(torch.cat([res_rot_norm, data.graph_sigma_emb], dim=1))

        if self.scale_by_sigma:
            tr_pred = tr_pred / tr_sigma.unsqueeze(1)
            rot_pred = rot_pred * so3.score_norm(rot_sigma.cpu()).unsqueeze(1).to(data['ligand'].x.device)

            res_tr_pred = res_tr_pred / res_tr_sigma.unsqueeze(1)
            res_rot_pred = res_rot_pred * so3.score_norm(res_rot_sigma.cpu()).unsqueeze(1).to(data['ligand'].x.device)


        if self.no_torsion or data['ligand'].edge_mask.sum() == 0: return tr_pred, rot_pred, torch.empty(0, device=self.device), res_tr_pred, res_rot_pred
