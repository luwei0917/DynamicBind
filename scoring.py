...
from utils.sampling import randomize_position, sampling, pred_lddt_and_affinity
...
args.keep_local_structures = True
...
test_dataset = PDBBindScoring(transform=None, root='', name_list=name_list, protein_path_list=protein_path_list, ligand_descriptions=ligand_descriptions,
                       ...
                       popsize=score_model_args.matching_popsize, maxiter=score_model_args.matching_maxiter,center_ligand=False,
                       ...
                       require_ligand=False,require_receptor=False, num_workers=args.num_workers, keep_local_structures=args.keep_local_structures)
score_batch_size = 2
test_loader = DataLoader(dataset=test_dataset, batch_size=score_batch_size, shuffle=False)
...
for idx, orig_complex_graph in tqdm(enumerate(test_loader)):
    ...
    try:
        ...
        try:
            ...
            all_lddt_pred_, all_affinity_pred_ = pred_lddt_and_affinity(orig_complex_graph, model, args.batch_size, device, score_model_args)
            # final_data_list.extend(orig_complex_graph)
            # all_lddt_pred_, all_affinity_pred_ = torch.tensor([1]), torch.tensor([1])
            all_lddt_pred.append(all_lddt_pred_)
            all_affinity_pred.append(all_affinity_pred_)
            # print(all_lddt_pred_, all_affinity_pred_)
        except Exception as e:
            # raise e
            print(idx, e)
            all_lddt_pred = [torch.zeros(score_batch_size)]
            all_affinity_pred = [torch.zeros(score_batch_size)]
        ...
        # write_dir = f'{args.out_dir}/index{idx}_{final_data_list[0]["name"][0].replace("/","-")}'
        write_dir = f'{args.out_dir}'
        os.makedirs(write_dir, exist_ok=True)
        # row = df.loc[df['name']==data_list[0]["name"][0]]
        # crystal_path = row['crystal_protein_path'].values[0]
        # protein_path = row['protein_path'].values[0]
        # ligand_path = row['ligand'].values[0]
        # shutil.copy2(f'{crystal_path}',f'{write_dir}')
        # shutil.copy2(f'{protein_path}',f'{write_dir}')
        # try:
        #     shutil.copy2(f'{ligand_path}',f'{write_dir}')
        # except:
        #     pass
        ...
        name_info.extend(orig_complex_graph.name)
        affinity_info.extend(all_affinity_pred)
        lddt_info.extend(all_lddt_pred)
        # final_affinity_pred = np.minimum((all_affinity_pred*all_lddt_pred).sum() / (all_lddt_pred.sum()+1e-12),15.)
        # final_lddt_pred = all_lddt_pred.mean()
        # affinity_pred[orig_complex_graph.name[0]] = final_affinity_pred
        # lddt_pred[orig_complex_graph.name[0]] = final_lddt_pred
        # info.append([orig_complex_graph.name[0], final_affinity_pred, final_lddt_pred])
...
affinity_pred_df = pd.DataFrame({'name':name_info,'affinity':affinity_info, 'lddt':lddt_info})
# affinity_pred_df = pd.DataFrame(info, columns=['name', 'affinity', 'lddt'])
